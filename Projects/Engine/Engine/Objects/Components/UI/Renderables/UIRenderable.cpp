...
namespace Engine
{
	...
	void UIRenderable::SetWidth(uint32_t Width)
	{
		...
		if (RenderBounds.width() == 0.0f || RenderBounds.height() == 0.f)
		{
			m_Bounds.fRight = glm::max((float)Width, m_MinSize.x);
		}
		else
		{
			m_Bounds.fRight = glm::max(glm::min((float)Width, RenderBounds.width()), m_MinSize.x);
		}
	...
	void UIRenderable::SetRenderBounds(const Vector2& Origin, const Vector2& Size)
	{
		Vector2 CalcSize;

		CalcSize.x = glm::min(Size.x, m_Width - Origin.x);
		CalcSize.y = glm::min(Size.y, m_Height - Origin.y);

		RenderBounds = SkRect::MakeXYWH(Origin.x, Origin.y, CalcSize.x, CalcSize.y);

		if (CalcSize.x == 0.0f || CalcSize.y == 0.f)
		{
			m_Bounds.fRight = glm::max((float)m_Width, m_MinSize.x);
			m_Bounds.fBottom = glm::max((float)m_Height, m_MinSize.y);
		}
		...
		{
			m_Bounds.fRight = glm::max(glm::min((float)m_Width, RenderBounds.width()), m_MinSize.x);
			m_Bounds.fBottom = glm::max(glm::min((float)m_Height, RenderBounds.height()), m_MinSize.y);
		}
	}
	...
	void UIRenderable::SetMinSize(const Vector2& Size)
	{
		...
		{
			m_Bounds.fRight = glm::max(glm::min((float)m_Width, RenderBounds.width()), m_MinSize.x);
			m_Bounds.fBottom = glm::max(glm::min((float)m_Height, RenderBounds.width()), m_MinSize.y);
		}
